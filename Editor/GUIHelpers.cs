...
namespace toxicFork.GUIHelpers
{
    public class GUIHelpers
    {
        ...
        public static bool CustomHandleButton(int controlID, Vector3 buttonPosition, float buttonSize, Material material)
        {
            ...
            switch (Event.current.type)
            {
                case EventType.repaint:
                    using (DisposableMaterialDrawer drawer = new DisposableMaterialDrawer(material)) {
                        float? prevHot = null;
                        if (material.HasProperty("_Hot")) {
                            prevHot = material.GetFloat("_Hot");
                            material.SetFloat("_Hot", GUIUtility.hotControl == controlID && distance <= 0 ? 1f : 0f);
                        }
                        drawer.DrawSquare(buttonPosition, Quaternion.identity, buttonSize);
                        if (prevHot != null)
                        {
                            material.SetFloat("_Hot", prevHot.Value);
                        }
    ...
    public class DisposableMaterialDrawer : IDisposable
    {
        private readonly Material material;
        ...
        public void DrawSquare(Vector3 position, Quaternion rotation, float size)
        {
            ...
            {
                material.SetPass(i);
                GUIHelpers.DrawSquare(position, this.rotation * rotation, scale*size);
            }
            GL.PopMatrix();
        }
    ...
    public class RotatedGUI : DisposableHelper
    {
        ...
        {
            disposables.Push(new CustomGUIMatrix(Matrix4x4.TRS(position + offset, Quaternion.identity, Vector3.one), true));
        }
