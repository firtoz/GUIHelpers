...
namespace toxicFork.GUIHelpers.Disposable {
	public class DisposablePullout : DisposableHelper {
		...
		public DisposablePullout(Rect screenRect, float minWidth, bool scrollable = false, Color? backgroundColor = null) {
			...
			disposables.Push(new DisposableGUILayoutArea(screenRect));
			...
			disposables.Push(new DisposableCustomViewport(new Rect(0, 0, screenRect.width, screenRect.height),
				new Rect(offset, 0, screenRect.width - offset, screenRect.height)));

			disposables.Push(new DisposableGUIMatrix(Matrix4x4.TRS(new Vector3(-offset, 0, 0), Quaternion.identity,
				new Vector3(1, 1, 1))));
			...
			if (scrollable) {
				disposables.Push(new DisposableEditorGUILayoutScrollView());
			}
